...
import { getPublicAppSettings } from "./app-settings.service.js";
...
const drawFooter = (doc: PDFKit.PDFDocument, company: { legalName: string; supportEmail: string; supportPhone: string }) => {
  doc.fillColor("#5b6474").font("Helvetica").fontSize(9);
  doc.text(
    `This is a computer-generated GST invoice issued by ${company.legalName}. For billing support contact ${company.supportEmail} or ${company.supportPhone}.`,
    40,
    FOOTER_Y,
    {
      width: 515,
...
export const generateInvoicePdfBuffer = async (orderId: string) => {
  ...
  doc.fillColor("#ffffff").font("Helvetica-Bold").fontSize(24).text(`${company.companyName} Tax Invoice`, 58, 54);
  doc.font("Helvetica").fontSize(10).fillColor("#dbe7f3").text(company.legalName, 58, 86);
  doc.text(`${company.addressLine1}, ${company.addressLine2}`, 58, 100, { width: 280 });

  doc.font("Helvetica-Bold").fontSize(10).fillColor("#c9f3ee").text("GSTIN", 390, 54);
  doc.font("Helvetica").fontSize(10).fillColor("#ffffff").text(company.gstin, 390, 68);
  doc.font("Helvetica-Bold").fontSize(10).fillColor("#c9f3ee").text("Support", 390, 88);
  doc.font("Helvetica").fontSize(10).fillColor("#ffffff").text(`${company.supportPhone} · ${company.supportEmail}`, 390, 102, {
    width: 140
  });
  drawStatusBadge(
    doc,
  ...
  if (declarationTop + 64 > PAGE_BOTTOM) {
    ...
    doc.font("Helvetica").fontSize(8).fillColor("#5b6474").text(
      `We declare that this invoice shows the actual price of the goods described and that all particulars are true and correct. This invoice is generated by ${company.legalName}.`,
      54,
      newDeclarationTop + 28,
      { width: 340, lineGap: 1 }
    );
    doc.font("Helvetica-Bold").fontSize(9).fillColor("#08111f").text(`For ${company.legalName}`, 412, newDeclarationTop + 16, {
      width: 120,
      align: "right"
    });
    ...
    doc.font("Helvetica-Bold").fontSize(9).fillColor("#08111f").text(`For ${company.legalName}`, 412, declarationTop + 16, {
      width: 120,
      align: "right"
    });
