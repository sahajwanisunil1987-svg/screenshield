...
export function RegisterForm() {
  ...
  const validate = () => {
    ...
    if (form.phone.trim().length < 10) nextErrors.phone = "Enter a valid phone number";
    if (form.password.length < 8) nextErrors.password = "Password must be at least 8 characters";
    if (form.confirmPassword.length < 8) nextErrors.confirmPassword = "Confirm your password";
  ...
  return (
    <AuthShell>
      ...
      <form onSubmit={onSubmit} className="mt-8 space-y-4">
        ...
        <Input type="email" placeholder="Email" autoComplete="email" value={form.email} onChange={(event) => updateField("email", event.target.value)} />
